...
getById = async (id) =>
  await sakila.select("*").from("inventory").where("inventory_id", id);

getWhere = async (column, value) =>
  await sakila.select("*").from("inventory").where(column, value);

insert = async (data) => await sakila("inventory").insert(data);

update = async (id, data) =>
  await sakila("inventory").where("inventory_id", id).update(data);
