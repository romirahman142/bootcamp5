...
getById = async (id) =>
  await sakila.select("*").from("language").where("language_id", id);

getWhere = async (column, value) =>
  await sakila.select("*").from("language").where(column, value);

insert = async (data) => await sakila("language").insert(data);

update = async (id, data) =>
  await sakila("language").where("language_id", id).update(data);
